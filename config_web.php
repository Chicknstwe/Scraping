...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  <title>Scraping Beta 0.4 - Configuración</title> <!-- Código hecho 100% por Demetrio Carmona Derqui. Imágenes de freepik.com libres de licencias. -->
</head>
<?php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if ($_REQUEST["web_new_url"] !== "" || $_REQUEST["web_new_name"] !== "") {
	  if ($_REQUEST["web_new_url"] == "" || $_REQUEST["web_new_name"] == "") {
		  $output = "Debes escribir nombre y url para añadir una nueva web.";
	  } else {
		  $exists = false;
		  ...
		  if (!$exists) {
			  ...
			  if ($websites[sizeof($websites) - 1] == $url &&  $names[sizeof($websites) - 1] == $name) {
				  $output = "La nueva url ha sido añadida correctamente.";
			  } else {
				  $output = "No ha sido posible añadir la url o el nombre.";
			  }	  
			  

		  } else {
			  $output = "El nombre o la url introducida ya existen en el registro.<br>";
		  }
	  }
	  ...
	  if (sizeof($websites) > $web_select) {
		  if ($websites[$web_select] != $delete_web_test && $names[$web_select] != $delete_name_test) {
			  $output = "La web seleccionada se ha borrado con éxito.\n";
		  } else {
			  $output = "La web seleccionada no se ha borrado correctamente. Es posible que se requiera modificar manualmente resources.php.\n";
		  }
      } else {
		  $output = "La web seleccionada se ha borrado con éxito.\n";
	  }
