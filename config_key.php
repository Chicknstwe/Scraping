  ...
  if ($_REQUEST["keyword_new"] !== "") {
		  ...
		  if (!$exists) {
			  ...
			  if ($keywords[sizeof($keywords) - 1] == $keyword) {
				  $output = "La nueva palabra clave ha sido añadida correctamente.\n";
			  } else {
				  $output = "No ha sido posible añadir la palabra clave.\n";
			  }	  
			  

		  } else {
			  $output = "La palabra clave introducida ya existe.\n";
		  }
	  ...
	  if (sizeof($keywords) > $keyword_select) {
		  if ($keywords[$keyword_select] != $delete_test) {
			  $output = "La palabra clave seleccionada se ha borrado con éxito.\n";
		  } else {
			  $output = "La palabra clave seleccionada no se ha borrado.\n";
		  }
	  } else {
		  $output = "La palabra clave seleccionada se ha borrado con éxito.\n";
	  }
